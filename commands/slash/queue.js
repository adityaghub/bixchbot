...
const command = new SlashCommand()
	...
	.setRun(async (client, interaction, options) => {
		...
		if (!player.queue.size || player.queue.size === 0) {
			...
			const queueEmbed = new MessageEmbed()
				.setColor(client.config.embedColor)
				.setDescription(`**<:oopsmusic:1014958946789904525> | Now playing:** [${ song.title }](${ song.uri })`)
				.addFields(
					{
						name: "Duration",
						value: song.isStream
			...
			if (player.queue.size < 11 || player.queue.totalSize < 11) {
				...
				const embedTwo = new MessageEmbed()
					.setColor(client.config.embedColor)
					.setDescription(
						`**<:oopsmusic:1014958946789904525> | Now playing:** [${ song.title }](${ song.uri }) [${ player.queue.current.requester }]\n\n**Queued Tracks**\n${ pages[page] }`,
					)
					.addFields(
						{
							name: "Track Duration",
