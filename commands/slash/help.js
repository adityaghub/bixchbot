...
const command = new SlashCommand()
	...
	.setRun(async (client, interaction) => {
		...
		helpEmbed.addField(
			...
			`Bixch` +
			"\n" +
			`[Home](${ client.config.supportServer }) | [ZaddY](https://discord.com/users/994573148810911805) | [Wanna Add Meeh?](https://discord.com/oauth2/authorize?client_id=${ client.config.clientId }&permissions=${ client.config.permissions }&scope=bot%20applications.commands)`,
		);
		...
		const getButtons = (pageNo) => {
			return new MessageActionRow().addComponents(
				new MessageButton()
					...
					.setEmoji("<:oopsleft:1015633040975872042>")
					.setStyle("PRIMARY")
					.setDisabled(pageNo == 0),
				new MessageButton()
					.setCustomId("help_cmd_but_1_app")
					.setEmoji("<:oops_play:1014952738720845824>")
					.setStyle("PRIMARY")
		...
		collector.on("collect", async (iter) => {
			...
			"\n" +
			`[Home](${ client.config.supportServer }) | [Issues](${ client.config.Issues }) | [Wanna Add Meeh?](https://discord.com/oauth2/authorize?client_id=${ client.config.clientId }&permissions=${ client.config.permissions }&scope=bot%20applications.commands)`,
			);
			await iter.update({
				embeds: [helpEmbed],
				components: [getButtons(pageNo)],
